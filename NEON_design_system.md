...
| **Last Sync**          | `metadata` -> `last_refreshed`                     | **Exact Target:** Targets the explicit past-tense JSON key with a cascading fallback to prevent "Data Offline" errors.               |
...
| **Active Card Background** | `#fef2f2` | Light Red Alert (High visibility for active region filter) |
| **Active Card Border**     | `#ef4444` | Solid Critical Red                                         |
| **Active Card Text**       | `#991b1b` | Dark Red (WCAG contrast compliant)                         |
...
### E. High-Density UI Compaction

- **The Strategy:** NOC dashboards require maximum information density without looking cluttered.
- **The Execution:** Sidebar `.neon-box` containers have been compacted (padding reduced from 18px to 12px, margins reduced from 20px to 12px). Form Selectboxes utilize a `-10px` negative margin to tighten the vertical footprint.
- **Active State Triggers:** Streamlit's `kind="primary"` button attribute is CSS-hijacked to turn the actively filtered region card into a high-contrast red alert state.
